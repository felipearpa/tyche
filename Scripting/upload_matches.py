...
parser.add_argument('--poolLayoutVersion', required=True, help='Pool layout version')

args = parser.parse_args()

csv_path = args.file
pool_layout_id = args.poolLayoutId
pool_layout_version = args.poolLayoutVersion
...
with open(csv_path, newline='', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        ...
        match_datetime = datetime.strptime(match_date, '%Y-%m-%dT%H:%M:%S.000Z').strftime('%Y-%m-%dT%H:%M:%S.000Z')

        match_item = {
            'pk': {'S': f'POOLLAYOUT#{pool_layout_id}'},
            'sk': {'S': f'MATCH#{match_id}'},
            'poolLayoutId': {'S': pool_layout_id},
            'matchId': {'S': match_id},
            'matchDateTime': {'S': match_datetime},
            'homeTeamName': {'S': local_team},
            'awayTeamName': {'S': away_team},
            'homeTeamId': {'S': local_team_id},
            'awayTeamId': {'S': away_team_id},
            'poolLayoutVersion': {'N': pool_layout_version},
        }
