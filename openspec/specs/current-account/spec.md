# current-account Specification

## Purpose
Keep one observable, offline-first current account per authenticated client: persist the account id, external account id, email, and username independently from authentication credentials, publish the persisted snapshot before any remote call, reconcile it against an authenticated current-account endpoint without discarding offline state, and order sign-in installation, refresh, username mutation, and logout clearing through one application-scoped coordinator.
