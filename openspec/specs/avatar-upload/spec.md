...
## Purpose
Let a gambler set a profile photo: pick or capture an image, crop it to a square, optimize it on the device, and upload it to S3 through a presigned URL scoped to their own account, following the `avatars/<accountId>.jpg` storage contract.
