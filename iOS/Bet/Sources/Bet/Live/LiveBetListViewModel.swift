...
public class LiveBetListViewModel: ObservableObject {
    private let getLivePoolGamblerBetsUseCase: GetLivePoolGamblerBetsUseCase
    ...
    public init(
        ...
        self.getLivePoolGamblerBetsUseCase = getLivePoolGamblerBetsUseCase
        self.gamblerId = gamblerId
        self.poolId = poolId

        let pagingSource = PoolGamblerBetPagingSource(
            pagingQuery: { next in
                await getLivePoolGamblerBetsUseCase.execute(
                    poolId: poolId,
                    gamblerId: gamblerId,
                    next: next,
                    searchText: nil
